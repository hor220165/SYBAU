<template>
  <div class="impressum-page">
    <div class="impressum-container">
      <button class="back-button" @click="goBack">
        Zurück
      </button>

      <div class="impressum-content">
        <h1 class="impressum-title">Impressum</h1>

        <section class="impressum-section">
          <h2>Projekt</h2>
          <p><strong>DidimDynamics</strong></p>
          <p>
            SYBAU ist ein Schülerprojekt der Spengergasse. Die App verbindet Training mit
            Avatar-Fortschritt, XP, Coins, Quests und einem Shop-System.
          </p>
          <p>
            Spengergasse 20<br>
            1050 Wien<br>
            Österreich
          </p>
        </section>

        <section class="impressum-section">
          <h2>Kontakt</h2>
          <p>
            Bei Fragen zum Projekt oder zu gespeicherten Daten ist das Team per E-Mail erreichbar:
            <a href="mailto:hor220165@spengergasse.at">hor220165@spengergasse.at</a>
          </p>
        </section>

        <section class="impressum-section">
          <h2>Team</h2>
          <div class="team-list">
            <div class="team-member">
              <span class="member-name">Viktor Horvath</span>
              <span class="member-role">Team Lead & Backend</span>
            </div>
            <div class="team-member">
              <span class="member-name">David Novakovic</span>
              <span class="member-role">Frontend</span>
            </div>
            <div class="team-member">
              <span class="member-name">Bakir Duric</span>
              <span class="member-role">Backend</span>
            </div>
            <div class="team-member">
              <span class="member-name">David Pfeiffer</span>
              <span class="member-role">Frontend</span>
            </div>
          </div>
        </section>

        <section class="impressum-section">
          <h2>Hinweis</h2>
          <p>
            SYBAU ist kein medizinisches Angebot. Trainingsdaten und Fortschritte dienen nur der
            Nutzung der App und ersetzen keine professionelle Beratung.
          </p>
        </section>

        <section class="impressum-section">
          <h2>Daten und Datenschutz</h2>
          <p>
            SYBAU speichert nur Daten, die für Account, Login, Profil, Training, Freunde,
            Fortschritt und Shop-Funktionen gebraucht werden. Die ausführliche Erklärung steht auf
            der Seite <RouterLink to="/datenschutz">Datenschutz</RouterLink>.
          </p>
        </section>

        <div class="impressum-footer">
          <p>© 2026 DidimDynamics</p>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { nextTick, onMounted } from 'vue';
import { useRouter } from 'vue-router';

const router = useRouter();

const scrollToTop = () => {
  window.scrollTo({ top: 0, left: 0, behavior: 'auto' });
  document.documentElement.scrollTop = 0;
  document.body.scrollTop = 0;
};

onMounted(async () => {
  scrollToTop();
  await nextTick();
  scrollToTop();
  window.requestAnimationFrame(scrollToTop);
});

const goBack = () => {
  router.back();
};
</script>

<style scoped>
.impressum-page {
  min-height: 100vh;
  padding: 80px 20px 40px;
  color: white;
}

.impressum-container {
  max-width: 720px;
  margin: 0 auto;
}

.back-button {
  position: fixed;
  top: 32px;
  left: 32px;
  background: none;
  border: none;
  color: rgba(255, 255, 255, 0.6);
  font-weight: 500;
  font-size: 14px;
  cursor: pointer;
  transition: all 0.3s ease;
  z-index: 100;
  padding: 0;
}

.back-button:hover {
  color: #ec4899;
  transform: translateX(-4px);
}

.impressum-content {
  background: transparent;
  padding: 0;
}

.impressum-title {
  font-size: 40px;
  font-weight: 700;
  margin: 0 0 48px 0;
  color: white;
  letter-spacing: -0.5px;
}

.impressum-section {
  margin-bottom: 48px;
  padding-bottom: 32px;
  border-bottom: 1px solid rgba(255, 255, 255, 0.1);
}

.impressum-section:last-of-type {
  border-bottom: none;
}

.impressum-section h2 {
  font-size: 18px;
  font-weight: 600;
  color: white;
  margin: 0 0 20px 0;
  text-transform: uppercase;
  letter-spacing: 0.5px;
  font-size: 14px;
  opacity: 0.6;
}

.impressum-section h3 {
  font-size: 16px;
  font-weight: 600;
  color: white;
  margin: 24px 0 12px 0;
}

.impressum-section p {
  font-size: 15px;
  line-height: 1.8;
  color: rgba(255, 255, 255, 0.7);
  margin: 0 0 16px 0;
}

.impressum-section a {
  color: #ec4899;
  text-decoration: none;
  transition: all 0.2s ease;
  border-bottom: 1px solid transparent;
}

.impressum-section a:hover {
  border-bottom-color: #ec4899;
}

.team-list {
  display: flex;
  flex-direction: column;
  gap: 16px;
}

.team-member {
  display: flex;
  justify-content: space-between;
  align-items: center;
  padding: 16px 0;
  border-bottom: 1px solid rgba(255, 255, 255, 0.05);
}

.team-member:last-child {
  border-bottom: none;
}

.member-name {
  font-size: 15px;
  font-weight: 500;
  color: white;
}

.member-role {
  font-size: 14px;
  color: rgba(255, 255, 255, 0.5);
}

.impressum-footer {
  margin-top: 64px;
  padding-top: 32px;
  border-top: 1px solid rgba(255, 255, 255, 0.1);
  text-align: center;
}

.impressum-footer p {
  font-size: 13px;
  color: rgba(255, 255, 255, 0.4);
  margin: 0;
}

/* Responsive */
@media (max-width: 768px) {
  .impressum-page {
    padding: 60px 20px 40px;
  }

  .back-button {
    top: 20px;
    left: 20px;
  }

  .impressum-title {
    font-size: 32px;
    margin-bottom: 40px;
  }

  .impressum-section {
    margin-bottom: 40px;
    padding-bottom: 24px;
  }

  .team-member {
    flex-direction: column;
    align-items: flex-start;
    gap: 4px;
  }
}
</style>
